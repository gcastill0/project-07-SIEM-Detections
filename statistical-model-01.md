...
- \( b_{u,t} \): outbound volume attributed to user \( u \) during period \( t \)  
- \( B_t = \sum_{u} b_{u,t} \): total outbound volume across all users during period \( t \)
...
- \( Z_u \approx 0 \): normal behavior  
- \( Z_u \approx 2 \): moderately unusual  
- \( Z_u \ge 3 \): statistically significant anomaly  

This approach detects **relative behavior changes**, not just high absolute traffic.
